...
def spectrograms(filename,window_size=1,channels=[0,1],samplerate_new=None):
    ...
    # len of the audio file in window_size
    len_in_secs = len(samples)/samplerate
    #print(len_in_secs)
    
    # len_in_secs/window_size is the number of timesteps to make spectogramms
    timesteps = int(len_in_secs//window_size)
    #print(timesteps)


    #print('timestep',window_size*samplerate)
    ...
    for c in channels:
        ...
        for i in range(timesteps):
            # start at timestep.
            start = samplerate*window_size*i
                
            # end one timestep later
            end = start+samplerate*window_size
